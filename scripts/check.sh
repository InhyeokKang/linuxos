...
ok = True
for f in glob.glob("config/**/*.xml", recursive=True) + glob.glob("config/**/*.conf", recursive=True):
...
ok = True
files = glob.glob("config/includes.chroot_after_packages/etc/calamares/**/*.conf", recursive=True) \
      + glob.glob("config/includes.chroot_after_packages/etc/calamares/**/*.desc", recursive=True) \
...
if command -v nft >/dev/null 2>&1; then
    echo "[+] nftables 문법"
    nft -c -f config/includes.chroot_after_packages/etc/nftables.conf 2>/dev/null || note "nft -c 실패 (include 경로 때문일 수 있음; 빌드 훅에서 재검사)"
fi
