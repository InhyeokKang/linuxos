...
- **배경**: 크림 종이 위 귤 단면 로고 하나 (정적 PNG, 비용 0). 파스텔 언덕 버전은 `branding/wallpaper-dawn.svg` 로 보관.
- **작업표시줄**: 바닥 전체 폭 한 줄 (윈도우 11 식). 양쪽 확장 구분선 사이에 [시작][고정 앱 + 실행 중 창 (docklike)]을 두어
  가운데 정렬하고, 오른쪽에 [트레이][빠른 설정][시계][알림][바탕화면 보기]. 오른쪽 묶음 폭만큼 가운데 묶음이 왼쪽으로
  조금 치우친다 (xfce4-panel 에는 화면 중심 정렬 옵션이 없다).
