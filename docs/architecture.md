...
- **배경**: 크림 종이 위 파스텔 곡선(새벽 언덕)과 귤 단면 로고 (정적 PNG, 비용 0). 단색 크림 버전은 `branding/wallpaper-plain.svg`.
- **작업표시줄**: 바닥 전체 폭 한 줄 (윈도우 11 식). xfce4-panel 에는 화면 중심 정렬 옵션이 없어 패널 두 개를 겹친다:
