...
- [x] 한글 입력: fcitx5 기본 프로필(두벌식 + 영어, 한/영·Shift+Space·오른쪽 Alt 전환) — 프리뷰와 실제 ISO 부팅 테스트(메모장 자동 입력 스크린샷)에서 "abc 한글 입력 테스트 end" 확인
...
- [x] Flatpak 최소 권한 기본값(호스트 파일시스템 차단, 사용자 폴더만) — 첫 부팅 적용
- [x] 실제 ISO: 설치 프로그램이 kiyu 프로필/버전으로 시작(브랜딩 확인), 한글 입력·차단목록 패키지 포함 확인 (1.58 GB)
- [x] kiyu Imager: USB/SD 굽기 GTK 도구(이동식 감지, pkexec dd, 검증) — 프리뷰 UI 검증, 실제 쓰기 미검증
- [ ] kiyu Imager 윈도우 판(크로스 플랫폼) — 첫 설치용
