...
| 윈도우 | kiyu | 비고 |
|---|---|---|
| Wi-Fi 연결 | 작업표시줄 Wi-Fi 아이콘 > 네트워크 선택 | 빠른 설정(Win+A) > Wi-Fi 옆 › > 네트워크 선택, 암호 입력 |
| 코덱 / NVIDIA 드라이버 | 자동 | 터미널에서 `kiyu-setup-codecs`, `kiyu-setup-nvidia` (Secure Boot 키 등록 안내 포함) |
| 업데이트 | 설정 > Windows 업데이트 | 설정 > 업데이트 (kiyu-update). 들어가면 자동 확인, [다운로드 및 설치] 한 번으로 시스템 + Flatpak 앱 갱신, 필요하면 [지금 다시 시작]. 보안 업데이트는 매일 자동 |
| 파일 탐색기 | 탐색기 (Win+E) | Thunar (Win+E). 자세히 보기 · 왼쪽 즐겨찾기(다운로드·문서·사진·음악·비디오) · 주소 경로 버튼 · 검색. 메뉴는 왼쪽 ⋮ |
| 작업표시줄 고정 | 작업표시줄 아이콘 우클릭 > 고정 | 고정 앱과 실행 중 창이 한 버튼, 실행 중이면 아래 점 표시 (docklike) |
| 파일 탐색기 | Thunar | 경로 표시줄, 주소 직접 입력, 휴지통, 외장 드라이브 자동 마운트, 윈도우 NTFS 파티션 읽기/쓰기 |
| Edge | 탱자 브라우저 | 추적기 차단·샌드박스 기본. Chromium 전용 사이트는 "Chromium 설치" 안내 |
| 메모장 | Mousepad | |
| 사진 | Ristretto | |
...
| Steam | Steam | 시작 메뉴 > kiyu 게임 설정 |
| .exe 실행 | Wine / Bottles | 시작 메뉴 > kiyu 윈도우 프로그램 설정 |
...
1. 시작 메뉴 > **kiyu 게임 설정** 실행 (Steam, Proton-GE, GameMode, MangoHud 설치. NVIDIA 면 드라이버 선택).
